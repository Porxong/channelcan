...
export default function Home() {
  ...
  const targetNumber = 10084;
  const incrementStep = 21;
  ...
  // Define the keys explicitly as a union type
  type StatKeys = 'orders' | 'members' | 'reviews';

  useEffect(() => {
    const incrementCounters = (key: StatKeys, target: number) => {
      return setInterval(() => {
        setCounts((prevCounts) => ({
          ...prevCounts,
          [key]: Math.min(prevCounts[key] + 31, target), // Increment by 10 (adjust as needed)
        }));
      }, 1); // Adjust the speed as needed
    };

    const ordersInterval = incrementCounters('orders', targets.orders);
    const membersInterval = incrementCounters('members', targets.members);
    const reviewsInterval = incrementCounters('reviews', targets.reviews);
  ...
  }, [targets.members, targets.orders, targets.reviews]);
  return (
    <Template>
      ...
      <div className="flex flex-col items-center">
        <h1 className="text-3xl font-bold">{counts.orders}+</h1>
        <p className="text-gray-400">Total Orders</p>
      </div>
      <div className="flex flex-col items-center">
        <h1 className="text-3xl font-bold">{counts.members}+</h1>
        <p className="text-gray-400">Number of members</p>
      </div>
      <div className="flex flex-col items-center">
        <h1 className="text-3xl font-bold">{counts.reviews}+</h1>
